...
void onEvent (ev_t ev) {
    ...
    switch(ev) {
        ...
        case EV_TXCOMPLETE:
            ...
            Serial.println(F("Starting next transmission "));
...
void do_send(osjob_t* j){
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
        ...
        int soundPin = A0;
        int numSamples = 50;
        int samplingInterval = 1100; // milliseconds
        int maxValue = 255;
        uint8_t sensorValues[numSamples]; // create buffer, an array of unsigned 8 bit integers
//        Serial.println(F(minute()));
        for (int i=0 ; i < numSamples ; i=i+1) {
          delay(samplingInterval);
          sensorValues[i] = constrain(analogRead(soundPin), 0, maxValue);
          Serial.println(F("Taking sample: "));
          Serial.println(i, DEC);
        }
//        Serial.println(F(sensorValues));
        LMIC_setTxData2(1, (byte *)sensorValues, numSamples, 0);
...
//        Serial.println(F("size of sensorValues"));
